...
let username_reg = regForm['username'];
let email_reg = regForm['email'];
...
for (var i = 0; i < trs.length; i++)
  (function (e) {
    trs[e].addEventListener("click", function () {
      ...
      vetEmail.type ='email';
      vetEmail.id ='success';
      vetEmail.style.border.color = "green";
      ...
      //btnVet.addEventListener('click',()=>vet(this.children[0].textContent,this.children[2].textContent,vetEmail.value));
...
function vet(title,usernametxt,email){

    email =  email.trim().toUpperCase();
    usernametxt =  usernametxt.trim().toUpperCase();

    $.post("vet.php", { title: title , username:usernametxt, vetEmail:email},
   function(data)
   {
     //alert(usernametxt);
     //alert(data);
...
loginForm.addEventListener('submit', sendData ,false); //Submit Login form
regForm.addEventListener('submit',register,false); //Submit Registeration
